...
parser.add_option('-b', '--blocksize', dest='blocksize', default=3,
                  help='blocksize')
parser.add_option('-p', '--use_pseudo', dest='use_pseudo', default=False,
                  help='Flag on whether or not to use pseudo-iterative refinement')
...
use_cholesky = int(options.use_cholesky)
use_pseudo = bool(int(options.use_pseudo))
...
if use_pseudo:
	mat += '/part-00000'
args = ['-mat ' + mat, '-blocksize ' + str(blocksize),
